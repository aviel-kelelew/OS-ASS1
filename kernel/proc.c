...
int rate = 5; // our change
int sjf = 0;// our change
int fcfs = 0; // our change
int def= 0; // our change

// our change 2 -- we initalize the global variables:

int sleeping_processes_mean = 0;    //system slepping in average
int runnable_process_mean =0;      //runnable time in average
int running_process_mean = 0;     //running time in average
int program_time = 0;   //sum all running time of all processes
int start_time =0 ;  //when the CPU started
int cpu_utilization = 0;    //sum total cpu running
int proc_so_far = 0;
...
userinit(void)
{
  ...
  // printf(" line 273 - p -> start runnable state %d\n", p->start_runnable_state);
...
exit(int status)
{
  ...
  //our change 2 -- update the averge statisics
  if(p->pid != 1 && p->pid != 2){
    // p->end_running_state = ticks;
    // p->running_time = p->end_running_state - p->start_running_state;
    update_state_time(p);
  ...
  p->xstate = status;
  //update_state_time(p); //our change 2
  p->state = ZOMBIE;
...
scheduler(void)
{
  ...
  for(;;){
    ...
    if(def==1){ // our change - default - need to change condition
      ...
      c->proc = min_p;
      int curr_ticks = ticks; // our change - measures time before contact switch
      ...
      release(&min_p->lock);
        ...
        for(p = proc; p < &proc[NPROC]; p++) {
          ...
          c->proc = min_p;
          swtch(&c->context, &min_p->context);
          //min_p->last_runnable_time = ticks; // our change - maybe need to catck lock
          // Process is done running for now.
          // It should have changed its p->state before coming back.
